...
def evaluate_metrics(model, dataloader, device):
    ...
    with torch.no_grad():
        for x, y, source_ids in dataloader:
            x, y = x.to(device), y.to(device)
            logits, _ = model(x, source_ids)
...
def get_preds_and_labels(model, dataloader, device):
    ...
    with torch.no_grad():
        for x, y, source_ids in dataloader:
            x, y, source_ids= x.to(device), y.to(device), source_ids.to(device)
            logits, aux_loss = model(x, source_ids)
            preds = torch.argmax(logits, dim=1)
            all_preds.append(preds.cpu())
            all_labels.append(y)
...
class MoELayer(nn.Module):
    def __init__(self, input_dim, expert_dim, num_experts, num_modalities, k=1, gate_noise_std=1.0):
        super().__init__()
        ...
        # Modality embedding for routing
        self.modality_embed = nn.Embedding(num_modalities, input_dim)
        self.router = nn.Linear(input_dim * 2, num_experts)  # [x; modality_emb] → logits

    def forward(self, x, modality_ids, return_topk=False):
        # Combine input features with modality embeddings
        modality_vecs = self.modality_embed(modality_ids)
        combined_input = torch.cat([x, modality_vecs], dim=-1)

        logits = self.router(combined_input)
        if self.training:
...
class MoEClassifier(nn.Module):
    def __init__(self, input_dim, expert_dim, num_experts, num_modalities, k, num_classes):
        super().__init__()
        self.moe = MoELayer(input_dim, expert_dim, num_experts, num_modalities, k)
        self.classifier = nn.Linear(input_dim, num_classes)

    def forward(self, x, modality_ids, return_topk=False):
        if return_topk:
            moe_out, aux_loss, topk = self.moe(x, modality_ids, return_topk=True)
            logits = self.classifier(moe_out)
            return logits, aux_loss, topk
        else:
            moe_out, aux_loss = self.moe(x, modality_ids)
            logits = self.classifier(moe_out)
...
def evaluate_with_expert_tracking(model, dataloader, device):
    ...
    with torch.no_grad():
        for x, _, tag_batch in dataloader:
            x = x.to(device)
            _, _, topk = model(x, tag_batch, return_topk=True)
